...
class VectorStoreService:
    ...
    def __init__(self, config: Optional[VectorStoreConfig] = None):
        ...
        self.llm = ChatOpenAI(
            ...
            callbacks=[self.opik_tracer] if self.opik_tracer else None
        )
    ...
    def query(self, query: str, response_mode: str = "compact") -> str:
        ...
        if "today" in query.lower():
            ...
            try:
                ...
                filtered_docs = [
                    doc for doc in all_potentially_relevant_docs
                    if doc.metadata.get("created_at", "").startswith(today_date_str)
                ]
            ...
            result_str = document_chain.invoke(
                {"context": filtered_docs, "input": llm_question},
                config=config
            )
            ...
            retriever = self.vector_store.as_retriever(
                search_kwargs={"k": 5}  # Standard k for general queries
            )
            ...
            response = retrieval_chain.invoke(
                {"input": llm_question}, # New chains expect "input"
                config=config
            )
