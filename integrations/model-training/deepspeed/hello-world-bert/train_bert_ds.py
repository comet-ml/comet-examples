...
def masking_function(
    ...
    # Now of the masked tokens, choose how many to replace with random and how
    # many to unmask
...
def load_model_checkpoint(
    ...
    msg = (
        f"Loading model and optimizer checkpoints done."
        f" Loaded from {latest_checkpoint_path}"
    )
    log_dist(
        msg,
        ranks=[0],
...
def train(
    ...
    # The Training Loop
    model_parameters_numbers = sum([p.numel() for p in model.parameters()])
    log_dist(
        f"Total number of model parameters: {model_parameters_numbers:,d}",
        ranks=[0],
        level=logging.INFO,
    )
    model.train()
    ...
    for step, batch in enumerate(data_iterator, start=start_step):
        ...
        if step % log_every == 0:
            ...
            if is_rank_0():
                # TODO: Is the loss automatically reported or not?
