...
from src.call_summarizer.models.models import CallCategory, CallSummary
...
from src.call_summarizer.services.vector_store import VectorStoreService, VectorStoreConfig
from src.call_summarizer.utils.file_utils import read_text_file, write_text_file
...
def show_summarize_call():
    ...
    uploaded_file = st.file_uploader(
        ...
        type=["txt", "md"],
        help="Upload a text or markdown file containing the call transcript"
    )
    
    # Or paste text
    transcript = st.text_area(
        "Or paste the call transcript here",
        height=200,
    ...
    selected_category = st.selectbox(
        "Select a category",
        category_names,
        index=category_names.index("other") if "other" in [c.lower() for c in category_names] else 0
    )
...
def show_manage_categories():
    ...
    with st.form("add_category"):
        ...
        prompt_template = st.text_area(
            "Prompt Template",
            help="Use {transcript} as a placeholder for the call transcript."
        )
