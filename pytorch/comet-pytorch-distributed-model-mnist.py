...
PROJECT_NAME = "pytorch-model-parallel"
...
def run_training_loop(rank, num_gpus, train_loader, test_loader):
    ...
    for epoch in range(1, epochs + 1):
            ...
            with dist_autograd.context() as cid:
                ...
                if rank == 1:
                    experiment.log_metric(
                        "batch_loss", loss.item(), step=(i + 1) * epoch
                    )
                ...
                assert (
                    remote_method(
                        ParameterServer.get_dist_gradients, net.param_server_rref, cid
                    )
                    != {}
                )
                opt.step(cid)
        ...
        acc = get_accuracy(test_loader, net)
        if rank == 1:
            experiment.log_metric("accuracy", acc, step=steps_per_epoch * epoch)
